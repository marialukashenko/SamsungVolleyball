...
public class ScreenComputerGame implements Screen {
    ...
    StaticBodyBox net;
    DynamicBodyPlayer person1;
    DynamicBodyComputer person2;
    ...
    public ScreenComputerGame(MyGdx myGdx) {
        ...
        block[0] = new StaticBodyBox(gdx.worldComputer, SCR_WIDTH / 2, 0, SCR_WIDTH, 0.3f); // пол
        block[1] = new StaticBodyBox(gdx.worldComputer, 0, MyGdx.SCR_HEIGHT / 2, 0.3f, 1000);
        block[2] = new StaticBodyBox(gdx.worldComputer, SCR_WIDTH, MyGdx.SCR_HEIGHT / 2, 0.3f, 1000);
        block[3] = new StaticBodyBox(gdx.worldComputer, SCR_WIDTH / 2, SCR_HEIGHT + 0.4f, SCR_WIDTH, 0.3f);
        net = new StaticBodyBox(gdx.worldComputer, SCR_WIDTH / 2, 1f, 0.2f, netHeight);
    ...
    public void render(float delta) {
        ...
        gdx.camera.update();
        gdx.worldComputer.step(1 / 60f, 6, 2);
        ScreenUtils.clear(0, 0, 0, 1);
        gdx.debugRenderer.render(gdx.worldComputer, gdx.camera.combined);
        ...
        if (isGoal && !isWin) {
            if (TimeUtils.millis() > timeGoal + timeInterval) {
                ...
                if (ball.getX() < SCR_WIDTH / 2) {
                    gdx.worldComputer.destroyBody(ball.body);
                    ball = new DynamicBodyBall(gdx.worldComputer, SCR_WIDTH / 4 * 3, ballHeight, 0.4f);
                    startGame = true;
